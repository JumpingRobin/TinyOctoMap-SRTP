...
// 初始化哈夫曼树: size设置为0，root设置为空(利用NULL_SEQ也即65535表示)，free下一处空闲设置为0
...
// *************************堆的push pop sift_up sift_down等操作*************************
...
uint16_t huffmanEnCode(uint8_t *data, uint16_t dataLength, HuffmanTree *tree, uint8_t *newData, uint16_t newDataMAXLength){
    /**
     * data为元素取值范围0~255的数组(8位...01...)
     */
    time_t startTime = time(NULL);

    dict_t dict;
    initDict(&dict); // 初始化字典: value--0 times--空(NULL_SEQ)
    // 统计data中的各元素值并建立字典
    if(!fillDictFromData(data, dataLength, &dict)){ // Q1. fillDictFromData()函数有些奇怪！
